...
import { ComponentType, Component, Context, Ref, VNode, createElement, ComponentChildren, RefObject, VUIEvent } from './vdom';
export * from './vdom';
...
declare type BuiltInEventSourceRefiners = typeof EVENT_SOURCE_REFINERS;
...
declare class EventApi {
    ...
    get extendedProps(): GenericObject;
}
...
interface ViewHookProps extends ViewProps {
    nextDayThreshold: Duration;
}
...
declare class Emitter<HandlerFuncs extends HandlerFuncTypeHash> {
    ...
    hasHandlers(type: keyof HandlerFuncs): number;
}
...
declare abstract class BaseComponent<Props = {}, State = {}> extends Component<Props, State> {
...
declare abstract class DateComponent<Props = {}, State = {}> extends BaseComponent<Props, State> {
    uid: string;
    largeUnit: any;
    prepareHits(): void;
...
interface RenderHookProps<HookProps> {
    hookProps: HookProps;
...
declare type HookPropsWithEl<HookProps> = HookProps & {
    el: HTMLElement;
};
declare type DidMountHandler<HookProps> = (hookProps: HookPropsWithEl<HookProps>) => void;
declare type WillUnmountHandler<HookProps> = (hookProps: HookPropsWithEl<HookProps>) => void;
interface MountHookProps<HookProps> {
    hookProps: HookProps;
    didMount: DidMountHandler<HookProps>;
    willUnmount: WillUnmountHandler<HookProps>;
...
declare class MountHook<HookProps> extends BaseComponent<MountHookProps<HookProps>> {
    rootEl: HTMLElement;
...
interface NowIndicatorHookProps {
    isAxis: boolean;
    date: Date;
    view: ViewApi;
}
...
interface SlotLaneHookProps extends Partial<DateMeta> {
    time?: Duration;
    date?: Date;
    view: ViewApi;
}
interface SlotLabelHookProps {
    time: Duration;
    date: Date;
    view: ViewApi;
    text: string;
}
interface AllDayHookProps {
    text: string;
    view: ViewApi;
}
interface DayHeaderHookProps extends DateMeta {
    date: Date;
    view: ViewApi;
    text: string;
    [otherProp: string]: any;
...
interface DayCellHookProps extends DateMeta {
    date: DateMarker;
    view: ViewApi;
...
declare class DayCellRoot extends BaseComponent<DayCellRootProps> {
    refineHookProps: (arg: DayCellHookPropsInput) => DayCellHookProps;
    normalizeClassNames: (generator: ClassNamesGenerator<DayCellHookProps>, hookProps: DayCellHookProps) => string[];
    render(): createElement.JSX.Element;
}
...
interface ViewRootHookProps {
    view: ViewApi;
}
declare class ViewRoot extends BaseComponent<ViewRootProps> {
    normalizeClassNames: (generator: ClassNamesGenerator<ViewRootHookProps>, hookProps: ViewRootHookProps) => string[];
    render(): createElement.JSX.Element;
}
...
currentState: object | null, action: Action | null, context: CalendarContext & CalendarDataManagerState) => object;
...
declare class CalendarContent extends Component<CalendarContentProps> {
    context: never;
...
declare type EventDropTransformers = (mutation: EventMutation, context: CalendarContext) => any;
...
declare type EventResizeJoinTransforms = (hit0: Hit, hit1: Hit) => false | object;
...
declare const EVENT_REFINERS: {
    extendedProps: Identity<GenericObject>;
...
declare function parseEventDef(refined: EventRefined, extra: GenericObject, sourceId: string, allDay: boolean, hasEnd: boolean, context: CalendarContext): EventDef;
...
declare function getEventClassNames(props: EventMeta): string[];
...
declare const BASE_OPTION_REFINERS: {
    ...
    dayHeaderClassNames: Identity<ClassNamesGenerator<DayHeaderHookProps>>;
    dayHeaderContent: Identity<CustomContentGenerator<DayHeaderHookProps>>;
    dayHeaderDidMount: Identity<DidMountHandler<DayHeaderHookProps>>;
    dayHeaderWillUnmount: Identity<WillUnmountHandler<DayHeaderHookProps>>;
    dayCellClassNames: Identity<ClassNamesGenerator<DayCellHookProps>>;
    dayCellContent: Identity<CustomContentGenerator<DayCellHookProps>>;
    dayCellDidMount: Identity<DidMountHandler<DayCellHookProps>>;
    dayCellWillUnmount: Identity<WillUnmountHandler<DayCellHookProps>>;
    ...
    weekNumberClassNames: Identity<ClassNamesGenerator<WeekNumberHookProps>>;
    weekNumberContent: Identity<CustomContentGenerator<WeekNumberHookProps>>;
    weekNumberDidMount: Identity<DidMountHandler<WeekNumberHookProps>>;
    weekNumberWillUnmount: Identity<WillUnmountHandler<WeekNumberHookProps>>;
    editable: BooleanConstructor;
    viewClassNames: Identity<ClassNamesGenerator<ViewRootHookProps>>;
    viewDidMount: Identity<DidMountHandler<ViewRootHookProps>>;
    viewWillUnmount: Identity<WillUnmountHandler<ViewRootHookProps>>;
    nowIndicator: BooleanConstructor;
    nowIndicatorClassNames: Identity<ClassNamesGenerator<NowIndicatorHookProps>>;
    nowIndicatorContent: Identity<CustomContentGenerator<NowIndicatorHookProps>>;
    nowIndicatorDidMount: Identity<DidMountHandler<NowIndicatorHookProps>>;
    nowIndicatorWillUnmount: Identity<WillUnmountHandler<NowIndicatorHookProps>>;
    ...
    now: Identity<string | number | number[] | Date | ((this: CalendarApi) => DateInput)>;
    eventDataTransform: Identity<EventInputTransformer>;
    ...
    eventClassNames: Identity<ClassNamesGenerator<EventMeta>>;
    eventContent: Identity<CustomContentGenerator<EventMeta>>;
    eventDidMount: Identity<DidMountHandler<EventMeta>>;
    eventWillUnmount: Identity<WillUnmountHandler<EventMeta>>;
    ...
    slotLaneClassNames: Identity<ClassNamesGenerator<SlotLaneHookProps>>;
    slotLaneContent: Identity<CustomContentGenerator<SlotLaneHookProps>>;
    slotLaneDidMount: Identity<DidMountHandler<SlotLaneHookProps>>;
    slotLaneWillUnmount: Identity<WillUnmountHandler<SlotLaneHookProps>>;
    slotLabelClassNames: Identity<ClassNamesGenerator<SlotLabelHookProps>>;
    slotLabelContent: Identity<CustomContentGenerator<SlotLabelHookProps>>;
    slotLabelDidMount: Identity<DidMountHandler<SlotLabelHookProps>>;
    slotLabelWillUnmount: Identity<WillUnmountHandler<SlotLabelHookProps>>;
    ...
    allDayClassNames: Identity<ClassNamesGenerator<AllDayHookProps>>;
    allDayContent: Identity<CustomContentGenerator<AllDayHookProps>>;
    allDayDidMount: Identity<DidMountHandler<AllDayHookProps>>;
    allDayWillUnmount: Identity<WillUnmountHandler<AllDayHookProps>>;
...
declare const VIEW_OPTION_REFINERS: {
    ...
    classNames: Identity<ClassNamesGenerator<ViewHookProps>>;
    content: Identity<CustomContentGenerator<ViewHookProps>>;
    didMount: Identity<DidMountHandler<ViewHookProps>>;
    willUnmount: Identity<WillUnmountHandler<ViewHookProps>>;
};
...
declare type DefaultedRefinedOptions<RefinedOptions extends GenericObject, DefaultKey extends keyof RefinedOptions> = Required<Pick<RefinedOptions, DefaultKey>> & Partial<Omit<RefinedOptions, DefaultKey>>;
...
declare const EVENT_SOURCE_REFINERS$1: {
    method: StringConstructor;
    extraParams: Identity<object | (() => object)>;
    startParam: StringConstructor;
    endParam: StringConstructor;
    timeZoneParam: StringConstructor;
};
...
declare type ExtraRefiners = typeof EVENT_SOURCE_REFINERS$1;
...
declare function memoizeObjArg<Arg extends object, Res>(workerFunc: (arg: Arg) => Res, resEquality?: (res0: Res, res1: Res) => boolean, teardownFunc?: (res: Res) => void): (arg: Arg) => Res;
...
declare function applyStyle(el: HTMLElement, props: object): void;
...
declare function requestJson(method: string, url: string, params: object, successCallback: any, failureCallback: any): void;
...
interface EventRootProps extends MinimalEventProps {
    ...
    children: (rootElRef: Ref<any>, classNames: string[], innerElRef: Ref<any>, innerContent: ComponentChildren, hookProps: EventMeta) => ComponentChildren;
}
...
export { Action, AllDayHookProps, AllowFunc, BASE_OPTION_DEFAULTS, BASE_OPTION_REFINERS, BaseComponent, BaseOptionRefiners, BaseOptionsRefined, BgEvent, BgEventProps, BusinessHoursInput, ButtonIconsInput, ButtonTextCompoundInput, CalendarApi, CalendarContent, CalendarContentProps, CalendarContext, CalendarData, CalendarDataManager, CalendarDataProvider, CalendarDataProviderProps, CalendarListenerRefiners, CalendarListeners, CalendarOptionRefiners, CalendarOptions, CalendarOptionsRefined, CalendarRoot, ChunkConfig, ChunkConfigContent, ChunkConfigRowContent, ChunkContentCallbackArgs, ClassNamesGenerator, ColGroupConfig, ColProps, Constraint, ConstraintInput, ContentHook, CssDimValue, CustomButtonInput, CustomContentGenerator, CustomContentRenderContext, DateComponent, DateEnv, DateFormatter, DateInput, DateMarker, DateMarkerMeta, DateMeta, DatePointApi, DatePointTransform, DateProfile, DateProfileGenerator, DateRange, DateRangeInput, DateSelectArg, DateSelectionApi, DateSpan, DateSpanApi, DateSpanInput, DateSpanTransform, DateUnselectArg, DayCellContent, DayCellContentProps, DayCellHookProps, DayCellRoot, DayCellRootProps, DayHeader, DayHeaderHookProps, DaySeriesModel, DayTableCell, DayTableModel, DayTableSeg, DelayedRunner, DidMountHandler, DragMeta, DragMetaInput, Duration, DurationInput, ElementDragging, ElementScrollController, Emitter, EventApi, EventClickArg, EventDef, EventDefHash, EventDropTransformers, EventHoveringArg, EventInput, EventInputTransformer, EventInstance, EventInstanceHash, EventInteractionState, EventMeta, EventMutation, EventRefined, EventRefiners, EventRenderRange, EventResizeJoinTransforms, EventRoot, EventSegUiInteractionState, EventSource, EventSourceApi, EventSourceDef, EventSourceHash, EventSourceInput, EventSourceRefined, EventSourceRefiners, EventStore, EventTuple, EventUi, EventUiHash, FormatterInput, GenericObject, Hit, Identity, Interaction, InteractionSettings, InteractionSettingsStore, LocaleInput, LocaleSingularArg, MinimalEventProps, MountHook, MountHookProps, NamedTimeZoneImpl, NowIndicatorHookProps, NowIndicatorRoot, NowIndicatorRootProps, NowTimer, OrderSpec, OverflowValue, OverlapFunc, ParsedRecurring, PluginDef, PluginDefInput, Point, PointerDragEvent, PositionCache, RawOptionsFromRefiners, Rect, RecurringType, RefMap, RefinedOptionsFromRefiners, RenderHook, RenderHookProps, RenderHookPropsChildren, ScrollController, ScrollGridImpl, ScrollGridProps, ScrollGridSectionConfig, ScrollRequest, ScrollResponder, Scroller, ScrollerLike, ScrollerProps, SectionConfig, Seg, SimpleScrollGrid, SimpleScrollGridSection, SlicedProps, Slicer, SlotLabelHookProps, SlotLaneHookProps, SplittableProps, Splitter, StandardEvent, StandardEventProps, TableDateCell, TableDowCell, Theme, ToolbarInput, VerboseFormattingArg, ViewApi, ViewComponentType, ViewContainerAppend, ViewContext, ViewContextType, ViewDef, ViewHookProps, ViewOptionRefiners, ViewOptionsRefined, ViewProps, ViewPropsTransformer, ViewRoot, ViewRootHookProps, ViewRootProps, ViewSpec, WeekNumberCalculation, WeekNumberHookProps, WeekNumberRoot, WeekNumberRootProps, WillUnmountHandler, WindowScrollController, addDays, addDurations, addMs, addWeeks, allowContextMenu, allowSelection, applyMutationToEventStore, applyStyle, applyStyleProp, asRoughMinutes, asRoughMs, asRoughSeconds, buildClassNameNormalizer, buildHashFromArray, buildNavLinkData, buildSegCompareObj, buildSegTimeText, collectFromHash, combineEventUis, compareByFieldSpec, compareByFieldSpecs, compareNumbers, compareObjs, computeEdges, computeFallbackHeaderFormat, computeHeightAndMargins, computeInnerRect, computeRect, computeSegDraggable, computeSegEndResizable, computeSegStartResizable, computeShrinkWidth, computeSmallestCellWidth, computeVisibleDayRange, config, constrainPoint, createDuration, createEmptyEventStore, createEventInstance, createEventUi, createFormatter, createPlugin, dateSelectionJoinTransformer, diffDates, diffDayAndTime, diffDays, diffPoints, diffWeeks, diffWholeDays, diffWholeWeeks, disableCursor, elementClosest, elementMatches, enableCursor, eventDragMutationMassager, eventTupleToStore, filterEventStoreDefs, filterHash, findDirectChildren, findElements, flexibleCompare, formatDate, formatDayString, formatIsoTimeString, formatRange, getAllowYScrolling, getClippingParents, getDateMeta, getDayClassNames, getDefaultEventEnd, getElSeg, getEventClassNames, getIsRtlScrollbarOnLeft, getRectCenter, getRelevantEvents, getScrollGridClassNames, getScrollbarWidths, getSectionClassNames, getSectionHasLiquidHeight, getSegMeta, getSlotClassNames, getStickyFooterScrollbar, getStickyHeaderDates, getUnequalProps, globalLocales, globalPlugins, greatestDurationDenominator, guid, hasBgRendering, hasShrinkWidth, htmlToElement, identity, interactionSettingsStore, interactionSettingsToStore, intersectRanges, intersectRects, isArraysEqual, isColPropsEqual, isDateSpansEqual, isElVisible, isInt, isInteractionValid, isMultiDayRange, isPropsEqual, isPropsValid, isSingleDay, isValidDate, listenBySelector, mapHash, memoize, memoizeArraylike, memoizeHashlike, memoizeObjArg, mergeEventStores, multiplyDuration, padStart, parseBusinessHours, parseClassNames, parseDragMeta, parseEventDef, parseFieldSpecs, parse as parseMarker, pointInsideRect, preventContextMenu, preventDefault, preventSelection, rangeContainsMarker, rangeContainsRange, rangesEqual, rangesIntersect, refineEventDef, refineProps, removeElement, removeExact, renderChunkContent, renderFill, renderMicroColGroup, renderScrollShim, requestJson, sanitizeShrinkWidth, setElSeg, setRef, sliceEventStore, sliceEvents, sortEventSegs, startOfDay, translateRect, triggerDateSelect, unpromisify, version, whenTransitionDone, wholeDivideDurations };
